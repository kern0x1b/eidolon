...
for scenario in "$@"; do
  if $gesture; then
    ...
    on_device "killall EidolonDemo 2>/dev/null; sleep 1; rm -f /var/charon/snapshots.on /var/charon/snapshots.png /var/charon/snapshots.only /var/tmp/eidolon-demo.log; printf %s $scenario > /var/charon/show; printf %s $scenario > /var/charon/gesture; chmod 666 /var/charon/show /var/charon/gesture; sblaunch -p space.kern0x1b.eidolon.demo" >/dev/null
  ...
  if $live; then
    ...
    on_device "killall EidolonDemo 2>/dev/null; sleep 1; rm -f /var/charon/snapshots.on /var/charon/snapshots.png /var/charon/snapshots.only; printf %s $scenario > /var/charon/show; chmod 666 /var/charon/show; sblaunch -p space.kern0x1b.eidolon.demo" >/dev/null
    sleep 8
    SECS=30 on_device "shot" >/dev/null
  ...
  on_device "killall EidolonDemo 2>/dev/null; sleep 1; rm -rf /var/charon/eidolon-snapshots /var/tmp/eidolon-demo.log; touch /var/charon/snapshots.on /var/charon/snapshots.png; printf %s $scenario > /var/charon/snapshots.only; chmod 666 /var/charon/snapshots.*; sblaunch -p space.kern0x1b.eidolon.demo" >/dev/null
